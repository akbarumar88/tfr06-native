...
$data = $conn
    ->query("SELECT barang.id, nama, kategori, harga, stock FROM barang 
            INNER JOIN kategori on kategori.id=barang.idkategori
            WHERE nama LIKE '%$cari%' LIMIT $entri OFFSET $offset")
    ->fetchAll();

$dataCount = $conn
    ->query("SELECT COUNT(id) as count FROM barang WHERE nama LIKE '%$cari%'")
    ->fetch()['count'];
...
<a href="/barang/create.php" class="btn btn-success"><i class="fa fa-plus"></i> Tambah</a>
<form action="<?= '' ?>/admin/barang/exportpdf" class="d-inline" method="POST">
    <input type="hidden" name="q" value="<?= $_GET['q'] ?>">
    <button type="submit" class="btn btn-warning"><i class="fa fa-print"></i> Export PDF</button>
</form>
<form action="<?= '' ?>/admin/barang/exportexcel" class="d-inline" method="POST">
    <input type="hidden" name="q" value="<?= $_GET['q'] ?>">
            ...
            <tbody>
                <?php foreach ($data as $i => $barang) : ?>
                    <tr>
                        ...
                        <td>
                            <div class="d-flex">
                                <a href="/barang/edit.php?id=<?= $barang['id'] ?>" class="btn btn-warning mr-2"><i class="fa fa-edit"></i> Edit</a>

                                <form action="/barang/hapus.php" method="POST">
